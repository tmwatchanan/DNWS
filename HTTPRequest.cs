...
namespace DNWS
{
  public class HTTPRequest
  {
      ...
      for(int i = 1; i != lines.Length; i++) {
        ...
        if(pair.Length == 1) { // handle post body
          if(pair[0].Length > 1) { //FIXME, this is a quick hack
            Dictionary<String, String> _bodysOriginal = pair[0].Split('&').Select(x => x.Split('=')).ToDictionary(x => x[0].ToLower(), x => x[1]);
            ConcurrentDictionary<String,string> _bodys = new ConcurrentDictionary<string, string>(_bodysOriginal);
    ...
    {
      _propertyListDictionary[key.ToLower()] = value.TrimEnd('\r', '\n');
    }
    public void AddRequest(String key, String value)
    {
      _requestListDictionary[key.ToLower()] = value.TrimEnd('\r', '\n');
    }
  }
}